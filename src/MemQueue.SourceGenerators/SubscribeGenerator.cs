...
namespace MemQueue.SourceGenerators;
...
public class SubscribeGenerator : IIncrementalGenerator
{
    ...
    private static SubscribeInfo ExtractSubscribeInfo(GeneratorAttributeSyntaxContext context)
    {
        ...
        var handlerInterface = classSymbol.AllInterfaces
            .FirstOrDefault(i => i.IsGenericType
                ...
                && i.ContainingNamespace.ContainingNamespace?.Name == "MemQueue");
    ...
    private static void GenerateRegistration(SourceProductionContext context, IList<SubscribeInfo> infos)
    {
        ...
        sb.AppendLine("using MemQueue.Abstractions;");
        sb.AppendLine("using MemQueue.Models;");
        sb.AppendLine();
        sb.AppendLine("namespace MemQueue;");
        sb.AppendLine();
        sb.AppendLine("public static partial class MemQueueSubscriberExtensions");
        sb.AppendLine("{");
        sb.AppendLine("    public static IServiceCollection AddMemQueueSubscribers(this IServiceCollection services)");
        sb.AppendLine("    {");
        ...
        foreach (var info in infos)
        {
            ...
            sb.AppendLine("        {");
            sb.AppendLine("            try { await _subscribeTask; } catch (Exception ex) { System.Diagnostics.Debug.WriteLine($\"MemQueue subscriber error: {ex.Message}\"); }");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine("}");
        ...
        context.AddSource("MemQueueSubscriberExtensions.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
    }
